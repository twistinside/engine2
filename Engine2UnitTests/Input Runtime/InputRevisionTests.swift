...
struct InputRevisionTests {
    ...
    @Test func codableRoundTripPreservesBothIdentityCoordinates() throws {
        let revision = InputRevision(session: .max, sequence: .max - 1)

        let encoded = try JSONEncoder().encode(revision)
        let decoded = try JSONDecoder().decode(InputRevision.self, from: encoded)
