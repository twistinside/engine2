...
struct InputRuntimeTests {
    ...
    @Test func standardAliasesDeduplicateAndOppositeDirectionsCancel() {
        ...
        runtime.receive(.keyDown(key(code: 13, name: "W")))
        runtime.receive(.keyDown(key(code: 126, name: "Up")))
        #expect(runtime.latestInputSnapshot.translation == SIMD2<Float>(0, 1))

        runtime.receive(.keyDown(key(code: 1, name: "S")))
        #expect(runtime.latestInputSnapshot.translation == .zero)

        runtime.receive(.keyUp(key(code: 13, name: "W")))
        #expect(runtime.latestInputSnapshot.translation == .zero)

        runtime.receive(.keyUp(key(code: 126, name: "Up")))
        #expect(runtime.latestInputSnapshot.translation == SIMD2<Float>(0, -1))
    }
    ...
    @Test func diagonalTranslationIsNormalizedAndReleasesRemainSemantic() {
        ...
        runtime.receive(.keyDown(key(code: 13, name: "W")))
        runtime.receive(.keyDown(key(code: 2, name: "D")))
        ...
        runtime.receive(.keyUp(key(code: 2, name: "D")))
        #expect(runtime.latestInputSnapshot.translation == SIMD2<Float>(0, 1))
    }
    ...
    @Test func spacePublishesHeldInteractionState() {
        ...
        runtime.receive(.keyDown(key(code: 49, name: "Space")))
        #expect(runtime.latestInputSnapshot.isInteractionActive)

        runtime.receive(.keyUp(key(code: 49, name: "Space")))
    ...
    private func key(code: UInt16, name: String) -> KeyboardKey {
        KeyboardKey(keyCode: code, displayName: name)
    }
