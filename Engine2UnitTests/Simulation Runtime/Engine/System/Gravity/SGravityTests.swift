...
struct SGravityTests {
    @Test func addsNewtonianAccelerationOnlyToExplicitReceiver() {
        ...
        world.gravitySourceComponents.insert(
            CGravitySource(gravitationalParameter: 5_000_000),
            for: star
        )
        ...
        world.gravityReceiverComponents.insert(CGravityReceiver(), for: skiff)
    ...
    @Test func sourceDoesNotAffectMovableEntityWithoutReceiverCapability() {
        ...
        world.positionComponents.insert(CPosition(position: .zero), for: star)
        world.gravitySourceComponents.insert(CGravitySource(gravitationalParameter: 5_000_000), for: star)
        world.positionComponents.insert(CPosition(position: SIMD3<Double>(100, 0, 0)), for: ordinaryMovable)
        world.motionComponents.insert(CMotion(), for: ordinaryMovable)
    ...
    @Test func twentyOrbitSoakRemainsFiniteAndWithinDriftLimits() {
        ...
        world.positionComponents.insert(CPosition(position: .zero), for: star)
        world.gravitySourceComponents.insert(CGravitySource(gravitationalParameter: gravitationalParameter), for: star)
        world.positionComponents.insert(CPosition(position: initialPosition), for: skiff)
        world.motionComponents.insert(CMotion(velocity: initialVelocity), for: skiff)
        world.gravityReceiverComponents.insert(CGravityReceiver(), for: skiff)

        var gravity = SGravity()
