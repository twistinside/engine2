...
struct SOrbitCircularizationAutopilotTests {
    ...
    @Test func appliesThrustAndFuelLimitsAcrossCompleteIntervals() throws {
        ...
        var system = SOrbitCircularizationAutopilot(
    ...
    @Test func productionForceOrderBlocksHeldManualThrustUntilCompletion() throws {
        ...
        var heldGravity = SGravity()
        var heldAutopilot = SOrbitCircularizationAutopilot(
            completionTolerance: completionTolerance
        )
        var heldFlightControl = SFlightControl(targetSpeed: 90, responseTime: 2)
        var heldMovement = SMovement()
        var neutralGravity = SGravity()
        var neutralAutopilot = SOrbitCircularizationAutopilot(
            completionTolerance: completionTolerance
        )
        var neutralFlightControl = SFlightControl(targetSpeed: 90, responseTime: 2)
        var neutralMovement = SMovement()
        let initialFuel = try #require(
    ...
    private func makeFixture(
        ...
        direction: COrbitCircularizationAutopilot.Direction = .counterclockwise
        ...
        world.gravityReceiverComponents.insert(CGravityReceiver(), for: entity)
