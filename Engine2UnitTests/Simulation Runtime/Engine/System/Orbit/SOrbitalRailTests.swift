...
struct SOrbitalRailTests {
    @Test func derivesPositionAndVelocityFromCompleteCircularPhase() {
        ...
        world.positionComponents.insert(CPosition(position: SIMD3<Double>(10, 20, 0)), for: primary)
        world.positionComponents.insert(CPosition(position: SIMD3<Double>(110, 20, 0)), for: satellite)
    ...
    @Test func twentyOrbitsRetainTheAuthoredRadius() {
        ...
        world.positionComponents.insert(CPosition(position: SIMD3<Double>(radius, 0, 0)), for: satellite)
        world.orbitalRailComponents.insert(
            COrbitalRail(
                primaryEntityID: primary,
                radius: radius,
