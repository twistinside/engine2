...
struct SCameraFollowTests {
    @Test func translatesCameraByTargetDisplacementWithoutChangingViewPolicy() {
        ...
        world.previousPositionComponents.insert(
            CPreviousPosition(position: SIMD3<Double>(100, 100, 0)),
            for: skiff
        )
        world.positionComponents.insert(CPosition(position: SIMD3<Double>(103, 96, 0)), for: skiff)

        var system = SCameraFollow()
