...
struct SimulationRuntimeAdvanceTests {
    ...
    @Test @MainActor func lifecycleTransitionsDoNotReplaceTheSession() {
        let simulation = makeSimulation()
    ...
    @Test @MainActor func activeRealtimeLoopRejectsASecondAdvanceAuthority() async {
        let simulation = makeSimulation()
        let cursor = simulation.currentCursor
        simulation.start()

        let outcome = await simulation.advance(
        ...
        #expect(
            outcome == .rejected(
                .advanceAuthorityActive(current: cursor)
            )
        )
