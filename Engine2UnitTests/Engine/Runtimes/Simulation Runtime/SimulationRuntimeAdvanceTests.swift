...
struct SimulationRuntimeAdvanceTests {
    ...
    @Test func sequentialRequestsDoNotReplaceTheSession() async throws {
        let simulation = makeSimulation()
        let sessionID = simulation.currentCursor.sessionID
        ...
        #expect(simulation.currentCursor.sessionID == sessionID)
    ...
    @Test func batchIngestsTransientInputOnlyOnItsFirstTick() async throws {
        ...
        #expect(simulation.world.input.translation == snapshot.translation)
    ...
    private struct OrbitCommandProbeBehavior: SimulationBehavior {
        func makeSystemSchedule() -> SimulationSystemSchedule {
            SimulationSystemSchedule(
                inputConsumption: [OrbitCommandProbeSystem()]
            )
        }
    }

    private struct OrbitCommandProbeSystem: System {
