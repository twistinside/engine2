...
struct FireableImpactSystemTests {
    ...
    @Test func unownedNonexpiringFiredBodyDestroysATarget() {
        ...
        let fired = DestructibleFireableTestEntity(
            in: scene.world,
            from: fireableState(destructible: DestructibleComponent())
        )
        scene.move(deltaTime: 1)
    ...
    @Test func nondestructibleFiredBodySurvivesADestructibleTarget() {
        ...
        let fired = IndestructibleFireableTestEntity(
            in: scene.world,
            from: fireableState(destructible: nil)
        )
        scene.move(deltaTime: 1)
    ...
    private func expiringTarget(
        ...
        ExpiringTargetTestEntity(
            ...
            from: Entity.InitialState(
                position: position,
                collisionBody: CollisionBodyComponent(radius: 1, restitution: 0),
                destructible: DestructibleComponent(),
                lifetime: LifetimeComponent(remainingLifetime: lifetime),
                previousPosition: PreviousPositionComponent(position: position)
            )
        )
    }

    private func fireableState(destructible: DestructibleComponent?) -> Entity.InitialState {
        Entity.InitialState(
            ...
            previousPosition: PreviousPositionComponent(position: SIMD3<Double>(10, 0, 0))
        )
    }
}
