...
struct MissileLaunchSystemTests {
    @Test func onePressRegistersOneMissileAndConsumesTheRequest() throws {
        ...
        let missileID = try #require(scene.world.missileComponents.entities.first)
        let missile = try #require(scene.world.entity(for: missileID) as? Missile)
        #expect(scene.world.missileComponents.entities.count == 1)
        ...
        #expect(scene.world.missileComponents.entities == [missileID])
    }
