...
- `AgentCaptureSource` and `AgentSessionAssembly` as the transport-neutral live-process wrapper that unifies bounded `.advance` and non-advancing `.current` admission and exact response replay while receiving only `POfflineCaptureTarget`
...
``MeshAssetKey`` and ``MaterialAssetKey``. The separate `BasicGameContent`
package owns closed `MeshID` and `MaterialID` enums, projects their cases to
those engine-owned keys, maps `MeshID.ball` to its packaged `Ball.usdz` asset,
and maps each material key to a ``PBRMaterialDescription``. The Render Runtime's
`MetalResourceStore` receives that catalog and privately owns the validated
descriptions, decoded meshes, buffers, compiled state, and residency
organization consumed by ``MetalFrameEncoder`` and its callers.
...
- an exact or alternate-output assembly may supply a separately owned output-specific `RenderViewpoint`
...
their single field declaration. The `Engine2GPUABI` SwiftPM target exposes the
production C declarations as an importable module for Swift, while Metal
Shading Language includes the same record headers directly. The reusable engine
package therefore does not depend on an App bridging header.
...
``PBRMaterialDescription``, Game Content identities such as `MaterialID`,
engine-owned transport keys, and validation policy remain ordinary Swift
values. ``RenderInstance`` owns transform projection and validation, while
focused Swift extensions pack its retained values and other validated semantic
inputs into imported transport records such as `GPUInstance`.
...
pattern. Engine2 packages its production layouts through the C-module boundary
described in
[TN3133](https://developer.apple.com/documentation/technotes/tn3133-packaging-a-renderer):
`Engine2GPUABI` exports the C declarations instead of exporting an App bridging
header as package API.
...
- decoded models resolved from backend-neutral ``MeshAssetKey`` values
- validated authored descriptions resolved from backend-neutral
  ``MaterialAssetKey`` values
...
`AgentSessionAssembly` privately retains an `OfflineCaptureAssembly` and gives
`AgentSessionCoordinator` only its
