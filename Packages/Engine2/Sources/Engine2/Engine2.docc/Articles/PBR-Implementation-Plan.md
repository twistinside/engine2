...
The validation scene reuses one sphere mesh for several material examples. The
`BasicGameContent` package therefore owns a strongly typed `MaterialID` as well
as `MeshID`; it projects both closed enums into package-neutral engine keys.
The engine must not invent duplicate mesh identities or decode the same
geometry multiple times to distinguish materials.

### Simulation Runtime

Simulation remains authoritative for semantic world state:

- `CRenderable` stores ``MeshAssetKey`` and ``MaterialAssetKey`` values.
  Snapshot capture iterates that store and joins position, rotation, and scale
  from their separate components.
...
- Project `MaterialID` into ``MaterialAssetKey`` during renderable spawn
  seeding, then carry that key through `CRenderable`, snapshot capture, and
  `RenderInstance` so the shared sphere mesh can represent several material
  appearances without an engine-to-content package dependency.
...
- Material factors and Metal resources never enter ECS or a Simulation
  snapshot. Only the package-neutral ``MaterialAssetKey`` crosses that
  boundary.
...
- Game Content owns the exhaustive `MaterialID` vocabulary. Milestone 4 began
  ...
  `MeshID.ball`. The separate Game Content package projects those values into
  ``MaterialAssetKey`` and ``MeshAssetKey`` before they enter engine contracts.
...
- ``RenderAssetCatalog`` maps every ``MaterialAssetKey`` to one description.
  Basic Game Content supplies `MaterialID.allCases.map(\.assetKey)` as the
  stable exhaustive validation order. Store construction validates that
  coverage before allocating or compiling backend resources. There is no
  default material and therefore no partially drawn frame with a substituted
  surface. `MetalSceneView.Coordinator` retains a construction failure for App
  diagnostics instead of silently erasing it when no renderer is created.
- With respect to authored materials, `CRenderable`,
  ``EntityPresentationSnapshot``, and `RenderInstance` carry only
  ``MaterialAssetKey``, not factors, compact GPU indices, buffers, or Metal
  objects. Snapshot capture copies the key by value, so a later ECS change
  cannot alter an already published presentation. `RenderInstance` separately
  retains its validated world, model-view, and normal-matrix projections for
  downstream reuse.
