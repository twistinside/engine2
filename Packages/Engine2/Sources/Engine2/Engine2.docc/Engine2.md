...
- The `Engine2` Swift package contains reusable Input, Simulation, Render, ECS, and Metal implementation. Basic Game Content, shared assembly support, and each concrete Runtime Assembly are separate packages.
- The App is the outer composition root. It imports `Engine2AssemblySupport`, its selected Game Content product, and one concrete assembly product; constructs and injects the content; retains one `PRuntimeAssembly`; and presents that assembly as its SwiftUI root. The assembly body owns any topology-specific presentation lifecycle.
...
- The `BasicGameContent` package supplies example entities, world construction, presentation descriptions, and assets without becoming a runtime or a dependency of `Engine2`.
- Game Content-owned `MeshID` and `MaterialID` enums project to package-neutral ``MeshAssetKey`` and ``MaterialAssetKey`` values before crossing engine-owned ECS, snapshot, and Render boundaries.
...
- compile-time package boundaries between reusable engine implementation, Basic Game Content, common assembly support, four concrete assembly products, and the thin App composition root
...
- a concrete serial offline capture assembly whose typed outcomes preserve either committed Simulation progress or the exact retained current presentation and, after rendering, the raw result needed for retryable artifact derivation
- a transport-neutral agent-session assembly whose closed operational surface exposes starting identity, `PAgentSessionTarget`, and explicit-host drain lifecycle while its common view presents static initial identity; focused coverage validates both capture sources through one admission/idempotency/cache/cursor/cancellation/lifecycle policy, and real integration advances to tick one, captures and replays an alternate view at tick one, then advances to tick two
