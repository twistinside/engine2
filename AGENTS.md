...
- `Engine2/Game Content/Star System Generation/**/*.swift`
  ...
  - V1 publishes survivors at or above `minimumResolvedPlanetSolidMassEarth` and retains the survivor with the greatest solid mass when none qualifies. Selection compares pre-moon embryo solid mass; validation compares the resolved parent-plus-moons solid mass because moon extraction preserves that total. The generator aggregates every omitted survivor into residual composition, body-count, and progenitor-count ledger fields; do not invent individual residual orbits, environments, moons, or classifications. This formed-body aggregate is distinct from unaccreted annulus solids, which do not represent a generated planetesimal, embryo, belt, or debris population.
