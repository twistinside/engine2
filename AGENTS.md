...
- `Engine2/Runtime Configuration/Agent/*.swift`
  ...
  - `AgentCaptureRequest` requires a session-qualified monotonic request identity, expected Simulation cursor, bounded positive step count, stable render request identity, viewpoint, render settings, and JPEG settings. The first slice deliberately submits `.none` input.
  ...
  - Identical retained requests replay the exact response and artifact; conflicts, in-flight duplicates, overlap, sequence gaps, wrong sessions, eviction, invalid payload, closure, and pre-acceptance cancellation are typed. Session and existing identity status are resolved before validating a new payload's reflexive equality, so a malformed retry still reports cached conflict, in-flight conflict, or eviction instead of being misclassified as new invalid work.
  - `AgentSessionLimits` bounds accepted steps, retained-result count, and retained raw/encoded image bytes. The named image-byte budget intentionally excludes snapshots and Swift object/collection overhead.
  ...
  - Idempotency is scoped to one live process. MCP transport, authentication, transport DTOs, restart-safe journals, physical or semantic controls, structured observation, artifact persistence, reset/load/fork operations, and content identity beyond current artifact provenance remain future work.
...
- `Engine2/Render Runtime/Frame/*.swift`
  ...
  - `RenderFrame.projectExact(from:viewpoint:)` is the strict request path. It rejects a malformed selected camera or any presented entity with missing position, an unusable finite normal-matrix inverse, or a nonfinite model-view transform instead of using the screen path's tolerant omission policy. `MetalOffscreenRenderRuntime` additionally validates the requested-aspect model-view-projection products before GPU packing.
...
- `Engine2UnitTests/`
  ...
  - `AgentSessionCoordinatorTests` exercises exact request mapping and at-most-once forwarding, retained replay, cached/in-flight conflict, duplicate-in-progress and unique-request busy outcomes, non-consuming admission and non-reflexive-payload rejection, accepted step limits, count/encoded-byte/raw-byte/oversize eviction, exact cursor derivation, accepted cancellation replay, close-and-drain with one or two simultaneous waiters, and maximum-sequence eviction after its successor becomes unrepresentable.
- `Engine2RenderTests/`
  ...
  - `AgentSessionConfigurationTests` drives production agent requests through only the closed assembly surface, proves an identical retained retry returns byte-identical JPEG data without advancing again, and then advances the next exact request sequence from the replayed response cursor.
...
- keep agent-session coordination above the narrow offline capture capability; never expose or inject a second Simulation advance or Render path, and never let bounded response-cache eviction make an accepted request executable again
...
- Typed multi-source input routing, route epochs, multi-window/output bindings, Simulation observer anchors, PNG output, artifact persistence/sinks, HDR-master and accumulation policy, and a dedicated Render worker remain proposed. The serial offline configuration coordinates exact advancement, rendering, provenance validation, and JPEG encoding; the agent configuration adds bounded live-process idempotency above it. Neither supplies actual MCP transport/authentication/DTOs, durable restart-safe request history, controls, structured observations, artifact persistence, automatic retry, Input, cadence, or a screen.
