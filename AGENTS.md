...
- `RealtimeSnapshotCaptureConnection` samples one completed Simulation presentation and derives a stable-identity, revision-zero explicit `RenderViewpoint` from that snapshot's camera. This conversion exists only because the exact offscreen request requires a viewpoint value; the connection owns no independently mutable camera state.
...
- `Engine2/Simulation Runtime/Engine/*.swift`
  ...
  - Input history, cleanup, acceleration intent, movement, and rotation are invariant members of every completed tick. The real-time screen camera can change only through a completed Simulation publication; deliberate exact output requests may still carry a separate viewpoint.
...
- `Engine2/Runtime Configuration/Offline/*.swift`
  ...
  - `OfflineCaptureCoordinator` awaits `PImageArtifactEncoder` while keeping its in-flight gate set, so reentrant overlap returns `.coordinatorBusy` during encoding too. The production `ImageIOArtifactEncoder` owns its immediately awaited detached CPU task. Caller cancellation is checked before encoding; once encoding begins, its completed artifact or typed failure wins.
...
- `Engine2/Render Runtime/Artifact/*.swift`
  ...
  - `ImageIOArtifactEncoder` is the stateless, `Sendable`, nonisolated production implementation. It immediately awaits detached CPU-side Core Graphics and Image I/O work for `.jpeg(quality:)` or lossless `.png`; once encoding begins, completion or typed failure wins over later caller cancellation.
...
- `Engine2/Render Runtime/View/*.swift`
  - `MetalSceneView` bridges SwiftUI to MetalKit input and drawing and wires separate presentation and input-sink capabilities selected by the App. The presentation snapshot supplies the screen camera.
...
`World.camera` and `SimulationPresentationSnapshot.camera` provide the completed camera used exactly by the real-time screen. The screen path has no independently mutable viewpoint source. If real-time camera input is added, it must enter Simulation through an attributable tick-boundary control path and become visible only in a completed presentation snapshot.
