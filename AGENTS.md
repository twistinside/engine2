...
- `SimulationReplayFile` is the versioned finite replay contract. `SimulationReplayDriver` constructs a fresh Simulation session from the selected Game Content and applies sparse exact Input assignments at their addressed consuming ticks; gaps assign `.none`, and the recorded session identity is provenance rather than authority over the new session.
- `RenderTrace` is the separate versioned semantic Render-input contract. It records complete Simulation presentations, either the Simulation camera or one explicit viewpoint, output settings, clear color, ordered frame identity, and content compatibility identity without persisting private Render frames, assets, Metal objects, or pixels.
- `RenderBenchmarkRunner` is the renderer-only trace workload host. It preloads content resources and private targets, then drives `MetalFrameEncoder` through a three-slot Metal 4 frame ring with queue feedback. It constructs no Input Runtime or Simulation Runtime, assembly, view, drawable, or readback path.

The current direct `InputMetalView`-to-`InputRuntime` connection is intentionally one concrete source connection, not a generalized routing framework. Multi-source input, typed routes and route epochs, multi-window/output bindings, Simulation observer anchors, image-artifact persistence/sinks, HDR accumulation, dedicated Render workers, and an actual MCP Runtime with transport, authentication, wire DTOs, durable idempotency, gameplay controls, and structured observations remain proposed. Agent current capture produces an exact visual artifact; it is not structured observation. The implemented pointer/scroll-to-camera mapping is one focused Simulation consumer, not a general physical or semantic control protocol.
...
- `BasicGameRecording` owns the example Game Content's recording compatibility identity. Change that identity whenever its tick-zero recipe, Simulation policy, identity vocabulary, or required assets become incompatible with an older replay or trace.
...
- `Engine2/Recorded Workflow/Simulation Replay/*.swift`
  - `SimulationReplayFile` stores one schema version, content compatibility identity, original session identity, optional initial Input baseline, terminal tick, and sparse exact assignments keyed by the completed tick that consumes them. The baseline restores held state without deriving transients.
  - `SimulationReplayDriver` validates content compatibility, constructs one fresh `SimulationRuntime`, and submits exact one-tick requests through the terminal tick. Its result retains tick zero plus every completed presentation in order.
  - A missing tick supplies `.none`, which applies no new assignment and does not clear previously held Input state.
  - The separate `SimulationReplay` command-line target uses an explicit source list without the App, assemblies, UI, renderer implementation, assets, or Metal. Its optional trace output projects the replay result into semantic Render input.
- `Engine2/Recorded Workflow/Render Trace/*.swift`
  - `RenderTrace` stores ordered semantic renderer input with complete Simulation presentations, viewpoint-selection policy, render settings, clear color, trace identity, and content compatibility identity.
  - `RenderTraceJSONReader` rejects unsupported schema versions before decoding the typed payload. Domain validation rejects malformed values and restores only trusted renderer input.
  - Trace records do not persist private `RenderFrame` values, decoded assets, Metal objects, ABI records, or pixels.
- `Engine2/Render Benchmark/*.swift`
  - The command host converts one trace into strict benchmark frames. `RenderBenchmarkRunner` preloads their Render resources and targets, then drives the production `MetalFrameEncoder` through three retained frame-resource slots.
  - Warm-up drains before measurement. Results separate CPU projection/preparation, CPU command recording/submission, GPU queue-feedback duration, and end-to-end wall throughput.
  - The separate optimized `RenderBenchmark` command-line target includes only the recorded content vocabulary and assets plus Render implementation. It has no Input Runtime or Simulation Runtime, assembly, UI, view, drawable, or readback.
...
- `Engine2/Input Runtime/**/*.swift`
  ...
  - The current `InputEvent` is host ingress, not a published ordered runtime event lane. Finite `SimulationReplayFile` values persist deliberately selected snapshot assignments; automatic live recording, complete ordered discrete transitions, and retained runtime delivery remain future work.
...
- `Engine2/Game Content/Recording/BasicGameRecording.swift`
  - Owns the example content's closed recording compatibility identity shared by committed Simulation replay and Render trace fixtures.
- `Engine2/Simulation Runtime/SimulationConfiguration.swift`
...
- `Engine2/Simulation Runtime/Snapshot/*.swift`
  ...
  - Ordinary live publication uses latest-value semantics. Finite replay results and Render traces retain explicit workflow values; a general live recorder or retained publication lane remains future work.
...
- `Engine2/Render Runtime/Metal/Offscreen/*.swift`
  ...
  - `MetalOffscreenRenderTargets` owns request-local shared BGRA8-sRGB destination and private depth textures plus their residency set. Successful readback produces an opaque, tightly packed, top-left `RenderedBGRA8SRGBImage`; the CPU-only Image I/O artifact layer consumes that detached value afterward, while HDR-master/accumulation, image-artifact persistence, and sinks remain future concerns.
- `Engine2/Render Runtime/Metal/Resource/*.swift`
  ...
  - Every `MetalResourceStore` construction explicitly selects a frame count. The screen and renderer-only benchmark deliberately pass `MetalResourceStore.defaultFrameCount` (currently `3`), while exact offscreen rendering passes `1`; target formats come from `MetalFrameEncoder`.
...
- `Engine2UnitTests/`
  ...
  - Recorded-workflow tests cover replay schema/domain validation, sparse tick application, content/session/cursor semantics, JSON round trips, semantic Render reconstruction, and malformed trace rejection.
- `Engine2RenderTests/`
  ...
  - Render benchmark tests cover iteration policy, workload projection, frame-ring reuse, queue-feedback completion and failure, timing statistics, and production encoder execution without a drawable or readback.
...
- Typed multi-source input routing, route epochs, multi-window/output bindings, Simulation observer anchors, image-artifact persistence/sinks, HDR-master and accumulation policy, additional artifact formats, atomic multi-view jobs, and a dedicated Render worker remain proposed. The serial offline assembly coordinates exact advance-or-current scene selection, rendering, provenance validation, and selected artifact encoding; the agent assembly adds bounded live-process idempotency above both sources. Neither supplies actual MCP transport/authentication/DTOs, durable restart-safe request history, controls, structured observations, image-artifact persistence, automatic retry, Input, cadence, or a screen. Visual current capture does not fill the structured-observation gap. Versioned Simulation replay and semantic Render trace files exist, but checkpoints, rollback, live recording, and pixel-output traces do not. Agent/MCP physical or semantic control ingress remains future and requires a deliberate typed agent-to-Simulation boundary; the focused real-time camera mapping is not that general protocol.
