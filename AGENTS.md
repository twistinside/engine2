...
- `Engine2/Game Content/Star System Generation/**/*.swift`
  ...
  - `.coreAccretionLiteV1` generates one main-sequence star, a conserved annular disk, bounded embryo accretion and migration, deterministic Hill clearing, present environments, and significant regular or impact moons. It derives orthogonal physical classifications only after resolving mass, composition, orbit, atmosphere, and temperature.
  ...
  - Formation computes every annulus claim from one pre-application snapshot and scales contested claims proportionally. Preserve this order-independence, the component-level solid ledger, and the aggregate hydrogen-helium ledger.
  ...
  - The canonical model and calibration documentation lives in `Star-System-Generation.md` and `Star-System-Generation-Calibration.md`. A formula, calibration, phase ordering, deterministic draw, or fallback change requires a new `StarSystemGenerationModelVersion`.
