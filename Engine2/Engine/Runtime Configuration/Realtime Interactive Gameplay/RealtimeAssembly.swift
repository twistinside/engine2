...
/// Its root view wires platform input, screen Render, debug UI, and scene
/// lifecycle without exposing those composition details to the App.
struct RealtimeAssembly: RuntimeAssembly, RealtimeAssemblyViewModel {
    ...
    var body: some View {
        RealtimeAssemblyView(assembly: self)
        .onAppear {
    ...
    var presentationSource: any SimulationPresentationSource {
        simulationRuntime
    }

    /// Platform ingress exposed without wider Input Runtime ownership.
    var inputSink: any InputEventSink {
        inputRuntime
    }

    /// Narrow selected-entity source for the topology-local SwiftUI inspector.
    var selectedEntitySource: any SelectedEntitySource {
        simulationRuntime
    }
    ...
    init(
        ...
        let inputRuntime = InputRuntime(
            mappingConfiguration: content.inputMappingConfiguration
        )
        let simulationRuntime = SimulationRuntime(
            worldBuilder: content.worldBuilder,
            configuration: content.simulationConfiguration,
            behavior: content.simulationBehavior,
        ...
        self.renderAssetCatalog = content.renderAssetCatalog
