...
struct RotationSystem: System {
    private static let signposter = OSSignposter(
        subsystem: "Engine2",
        category: "RotationSystem"
    )

    mutating func update(world: inout World, deltaTime: Double) {
        let signpostState = Self.signposter.beginInterval("RotationSystem.update")
        defer {
            Self.signposter.endInterval("RotationSystem.update", signpostState)
        }
        ...
        let zeroAccumulator = AngularMotionAccumulatorComponent(
