...
struct RenderInstance: Equatable {
    ...
    /// Camera-relative transform proved finite during frame projection.
    let modelViewMatrix: simd_float4x4
    /// Inverse-transpose linear transform proved usable with the model-view value.
    let normalMatrix: simd_float3x3

    /// Projects one entity through a selected camera view when its transforms are usable.
    init?(projecting entity: EntityPresentationSnapshot, viewMatrix: simd_float4x4) {
        ...
        // inverse can overflow. Validate the normal-matrix operation
