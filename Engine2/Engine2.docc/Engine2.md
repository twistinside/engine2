...
- ``GravitySystemGenerator`` projects that validated system into versioned planar Kepler rails with deterministic phase and orientation. ``GravitySystemEphemeris`` evaluates the generated hierarchy through the content-neutral ephemeris implementation.
- ``GeneratedStarSystemWorldBuilder`` materializes resolved stars, planets, and moons as authoritative celestial ECS state. Content-neutral ephemeris evaluation and versioned velocity-Verlet mechanics support fixed-tick orbital advancement without claiming generated-system N-body stability.
