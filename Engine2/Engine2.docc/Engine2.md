...
- ``SimulationReplayFile`` stores exact tick-addressed Input assignments, and ``SimulationReplayDriver`` applies them to a fresh Simulation session. ``RenderTrace`` separately stores semantic renderer input so Render work can run without Input or Simulation.
...
- a versioned file-backed Simulation replay driver that reconstructs a fresh session from tick-zero content, applies sparse exact Input assignments at their consuming ticks, and retains every completed presentation
- a versioned semantic ``RenderTrace`` plus a separate optimized Metal command-line benchmark that preloads resources and renders the trace through the production encoder without a view, drawable, Input Runtime, Simulation Runtime, or pixel readback

The obsolete `SimulationLoop`, elapsed-time Engine adapter, partial-schedule pause gate, and presentation-side camera bypass have been removed. ``SInputMapping`` and ``SCameraInput`` now form one focused Simulation-owned control path: imported pointer and scroll transients become semantic orbit/zoom commands, `World.camera` changes only within a complete tick, and the real-time screen observes that camera only after completed publication. Exact raw offscreen rendering, explicit request-carried viewpoints, CPU-side JPEG and PNG derivation, serial advance-or-current capture, the live-process idempotent agent wrapper, file-backed Simulation replay, semantic Render traces, and renderer-only trace benchmarking are implemented. The agent layer has no automatic cadence and preserves the offline coordinator as the only advance authority. Its current-cursor image artifact is visual output, not structured observation. An actual MCP Runtime or transport, authentication, wire DTOs, restart-safe agent idempotency, general physical or semantic gameplay controls, structured observations, image-artifact sinks, dedicated render workers, atomic multi-view jobs, high-quality accumulation/HDR policy, additional artifact formats, typed routing, multi-window bindings, checkpoints, and observer anchors remain proposed; advancing agent requests currently assign `.none`.
## Topics
### Architecture
