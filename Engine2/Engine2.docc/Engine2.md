...
- an assembly-retained Input Runtime that maps physical bindings to held translation and interaction intent plus cumulative camera and selection values; ``RealtimeAdvanceDriver`` captures its immutable latest snapshot and assigns it to an exact Simulation request
...
The obsolete `SimulationLoop`, elapsed-time Engine adapter, partial-schedule pause gate, and presentation-side camera bypass have been removed. ``InputRuntime`` now owns physical-to-semantic mapping and publishes context-free translation, interaction, camera, and selection intent. Simulation derives interval deltas, resolves selection, and routes entity controls against authoritative ECS state; `World.camera` still changes only within a complete tick. The mining slice keeps the skiff dynamically integrated under gravity and thrust while the star is its gravity source and quiet asteroids and depot follow deterministic analytic rails. Its SwiftUI selected-entity inspector reads protocol-backed live capabilities through a narrow Simulation-owned source rather than expanding ``SimulationPresentationSnapshot`` with gameplay state.
