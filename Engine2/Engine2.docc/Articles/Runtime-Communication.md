...
| --- | --- | --- |
| Input Runtime | held translation and interaction intent plus cumulative camera, selection, and fire values | future ordered semantic transitions or text input |
...
The implemented input boundary demonstrates latest-value behavior. `InputRevision` identifies the publisher session and version represented by each `InputSnapshot`. Held translation and interaction intent persist until a later publication changes them. Camera orbit and zoom are cumulative totals, and the latest normalized selection press is paired with a cumulative press count. Fire intent uses a separate cumulative count that advances once per mapped key press, ignores held-key repeats, and survives key release. Simulation can therefore derive the complete camera interval and detect selection and fire presses even when host events and fixed ticks do not run one-for-one. Multiple selection or fire presses between imports coalesce into one request for that interval; this boundary does not retain their order or occurrence history. Re-reading the same revision does not replay a transient delta or press. A rebase consumes historical press counts without firing, and a new Input Runtime session starts fresh counts. When the platform surface loses keyboard focus or its window resigns key status, it submits a physical focus-loss event. If a key is held, ``InputRuntime`` clears its held-key state and republishes the resulting semantic value so a missed key-up cannot leave gameplay input stuck.
