...
- ``RenderFrame.extract(from:)`` as the simulation-to-render snapshot boundary
...
## Render Extraction
The current renderer already consumes a flat `RenderFrame` of `RenderInstance` values extracted from ECS state.
