...
every produced JPEG is decoded and checked. Modularity is
...
- ``OfflineCaptureCoordinator`` has no automatic cadence; one accepted advance-or-current workflow owns the shared gate through exact scene selection, Render, and the awaited out-of-actor JPEG work. Overlap receives immediate busy refusal, including across the two operation kinds.
...
- `SInputHistory` and `SMovement` are scheduled Simulation logic, while output-specific camera control is App-owned presentation behavior.
...
| ``RealtimeSnapshotCaptureConnection`` | Implemented App-owned current-presentation selection and directed offscreen JPEG workflow; samples one exact Simulation snapshot and matching screen viewpoint without advancing or pausing Simulation |
