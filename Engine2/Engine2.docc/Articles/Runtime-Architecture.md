...
| ``AgentCaptureSource``, ``AgentSessionConfiguration``, ``AgentSessionAssembly``, and ``AgentSessionCoordinator`` | Implemented transport-neutral live-process wrapper that privately retains the offline assembly and receives only ``POfflineCaptureTarget``; it unifies `.advance` and `.current` under mandatory expected cursors, reflexive-payload admission, session-qualified monotonic identity, explicit accepted high-water independent of bounded replay retention and optional next-sequence state, exact cached response replay, source-appropriate step bounds, typed overlap/admission outcomes, and drain-before-close lifecycle |
