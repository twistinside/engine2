...
The standalone ``JPEGArtifactEncoder`` still chooses no executor. This particular coordinator chooses to run production encoding in an immediately awaited detached task outside its actor. The task deliberately does not inherit caller cancellation, so once encoding starts its result wins; the coordinator retains its single-flight gate while awaiting it. A future MCP coordinator may reuse these narrow capabilities but may choose different scheduling policy. No MCP transport/idempotency layer, persistence boundary, or `ArtifactSink` is implemented yet.
