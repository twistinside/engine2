...
The interactive App also composes one dedicated offscreen Metal Runtime beside its real-time assembly. ``RealtimeSnapshotCaptureConnection`` synchronously selects the latest completed Simulation presentation before its first suspension. Because ``POffscreenRenderTarget`` deliberately requires an explicit viewpoint value, the connection derives one with a stable connection identity, revision zero, and the selected snapshot's exact camera. It owns no independently mutable camera state. The connection then carries both immutable values through the shared offscreen artifact derivation without pausing or advancing Simulation. `Engine2App` retains the concrete real-time assembly as ordinary App-owned lifetime state, while `ContentView` receives only the presentation, input, advancement-state, history-read, and capture capabilities it actually uses. The current UI deliberately requests maximum-quality JPEG from its 4K button and presents SwiftUI's user-selected file exporter only after that detached JPEG artifact exists; disk destination policy remains outside Render.
...
| ``RealtimeConfiguration``, ``RealtimeAssembly``, and ``RealtimeAdvanceDriver`` | Implemented real-time composition with App-owned cadence, pause policy, exact requests, coordinated lifecycle, and direct App wiring from `InputMetalView` to ``InputRuntime``; broader authority recovery and typed routing remain |
| ``RealtimeSnapshotCaptureConnection`` | Implemented App-owned current-presentation selection and directed offscreen artifact workflow; samples one exact Simulation snapshot and derives the explicit offscreen viewpoint with a stable identity, revision zero, and that snapshot's exact camera without advancing or pausing Simulation |
...
| ``OfflineCaptureConfiguration``, ``OfflineCaptureAssembly``, and ``OfflineCaptureCoordinator`` | Implemented closed serial topology exposing only initial cursor plus ``POfflineCaptureTarget``; the sole effective advance authority retains one exact completed presentation and offers at-most-once advance capture plus mandatory-cursor current capture through one gate, shared Render/artifact validation, awaited encoder-owned out-of-actor work, and source-specific typed progress |
| ``AgentCaptureSource``, ``AgentSessionConfiguration``, ``AgentSessionAssembly``, and ``AgentSessionCoordinator`` | Implemented transport-neutral live-process wrapper that privately retains the offline assembly and receives only ``POfflineCaptureTarget``; it unifies `.advance` and `.current` under mandatory expected cursors, reflexive-payload admission, session-qualified monotonic identity, value-semantic sequence progress independent of bounded replay retention, exact cached response replay, source-appropriate step bounds, typed overlap/admission outcomes, and drain-before-close lifecycle |
