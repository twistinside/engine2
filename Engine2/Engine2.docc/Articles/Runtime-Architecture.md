...
The camera carried by ``SimulationPresentationSnapshot`` is the completed Simulation-authored camera used exactly by the real-time screen. The live screen has no separate viewpoint authority. A future gameplay-authoritative camera rig or input-driven camera therefore remains ordinary Simulation state processed by complete ticks and published only after completion. Deliberate exact outputs—including offline and agent captures—may still supply a separately owned ``RenderViewpoint`` by value without changing ``World`` or advancing the Simulation cursor; that request policy does not reopen a raw-input path into the live screen.
...
Artifact derivation begins only after that exact result exists. ``OffscreenImageArtifactDeriver`` validates the raw result and passes it with the exact ``ImageArtifactEncoding`` to an injected ``PImageArtifactEncoder``. ``ImageIOArtifactEncoder`` asynchronously derives JPEG or PNG bytes in immediately awaited detached CPU work and returns ``RenderedImageArtifact`` with the source request identity, Simulation cursor, complete viewpoint, render settings, and encoding intact. It is an ordinary stateless value rather than a Runtime. Encoding failure or artifact-provenance mismatch leaves the raw result available for retry and never implies another Simulation advance or render.
...
- ``PImageArtifactEncoder`` has no cadence; each implementation owns its execution context. Production ``ImageIOArtifactEncoder`` performs one immediately awaited detached CPU transformation per call.
...
- `SInputHistory` and `SMovement` are scheduled Simulation logic. A future input-driven real-time camera would also be Simulation behavior, while an explicit exact-output viewpoint remains request policy outside the scheduler.
