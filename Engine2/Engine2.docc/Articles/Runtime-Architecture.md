...
Artifact derivation begins only after that exact result exists. ``OffscreenImageArtifactDeriver`` validates the raw result and passes it with the exact ``ImageArtifactEncoding`` to an injected ``PImageArtifactEncoder``. ``ImageIOArtifactEncoder`` asynchronously derives JPEG or PNG bytes through an `@concurrent` operation on Swift's concurrent executor and returns ``RenderedImageArtifact`` with the source request identity, Simulation cursor, complete viewpoint, render settings, and encoding intact. It is an ordinary stateless value rather than a Runtime. Encoding failure or artifact-provenance mismatch leaves the raw result available for retry and never implies another Simulation advance or render.
...
| `InputState` and default Simulation input systems | Simulation-owned fixed-tick input state with raw-to-semantic camera mapping, authoritative orbit/zoom application, history, and raw/mapped transient cleanup after snapshot ingestion |
