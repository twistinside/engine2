...
Reusable Metal frame encoding and the first production offscreen Runtime boundary are now view-independent. ``MetalFrameEncoder`` owns material preflight, fixed target formats, frame-buffer packing, backend state binding, the HDR pass, and model draws while callers own targets and submission policy. ``MetalRenderer`` is the thin MetalKit screen caller. ``MetalOffscreenRenderRuntime`` implements the backend-neutral async ``POffscreenRenderTarget`` capability using request-carried immutable Simulation presentation, an explicit viewpoint, settings, configurable limits, dedicated one-slot resources, strict presentation/model/geometry preflight, real queue-feedback lifetime, and detached raw BGRA8-sRGB results. It samples no source, advances no Simulation, and owns no view or drawable. ``JPEGArtifactEncoder`` is the implemented stateless CPU transformation from that detached result to a provenance-rich JPEG; it selects no execution context and can be retried without ticking or rerendering. ``OfflineCaptureConfiguration`` composes those exact capabilities with Simulation behind one serial ``POfflineCaptureTarget``; its coordinator alone may advance that session. The coordinator verifies requested image extent and post-submission cancellation identity before accepting Render correlation, then awaits production JPEG work outside its actor without inheriting caller cancellation.

``AgentSessionConfiguration`` now adds a transport-neutral live-session boundary without reopening that topology. Its ``AgentSessionAssembly`` privately retains an ``OfflineCaptureAssembly`` and gives ``AgentSessionCoordinator`` only ``POfflineCaptureTarget``. Session-qualified monotonic request identities, mandatory expected cursors, bounded steps, stable capture payloads, at-most-once forwarding, bounded exact-response replay, typed overlap/rejection, and drain-before-close lifecycle therefore sit above the same sole effective advance authority. New non-reflexive payloads such as a NaN camera are refused without consuming their sequence after existing identity status is resolved. Accepted high-water is stored separately from both retained responses and the optional next sequence, preserving `.resultEvicted` even for an unretained maximum-sequence result. Sixteen focused unit cases cover admission, idempotency, encoded/raw retention, cursor, cancellation, and single/multi-waiter lifecycle behavior; real integration coverage executes two sequential unique agent requests and proves an identical retained retry returns the byte-identical JPEG response without advancing again. Broader authority recovery, typed routing, controls, structured observation, multi-output bindings, observer anchors, PNG/HDR accumulation, artifact persistence, dedicated Render isolation, and an actual MCP Runtime or transport remain proposed.
...
JPEG derivation is not another directed Runtime capability. It is a local transformation of an already completed value. ``OfflineCaptureCoordinator`` now submits one supplied exact advance request at most once, renders only the returned immutable snapshot, and validates that a completed result echoes every identity-bearing input plus the requested pixel size. Post-submission cancellation must also echo the expected request identity; mismatch is a typed outcome that retains the committed advance and both expected/actual IDs. The request may contain one or more fixed steps, and the coordinator never retries or rolls back automatically.
...
``AgentSessionCoordinator`` does not reproduce that workflow or receive its component capabilities. It forwards each accepted ``AgentCaptureRequest`` through ``POfflineCaptureTarget`` at most once, then retains the exact ``AgentSessionResponse`` within configured entry-count and raw/encoded-image-byte budgets. Accepted sequence high-water is independent of that cache and remains recorded when no next `UInt64` sequence exists: an evicted old request, including an unretained maximum-sequence result, receives `.resultEvicted` and can never advance again. ``AgentSessionRequestSequence/successor()`` returning `nil` is the typed exhaustion signal. Before accepting a fresh request, the coordinator also requires reflexive payload equality; existing retained, active, or evicted identity status takes precedence over that validation. Retained identical requests replay even after session closure, while new unique work is refused once `stopAndDrain()` begins. This idempotency exists only for the live assembly; there is no durable journal, MCP transport, persistence boundary, or `ArtifactSink` yet.
...
- ``OfflineCaptureCoordinator`` has no automatic cadence; one accepted request owns the complete advance-render-encode sequence, while overlapping actor-reentrant requests receive immediate busy refusal during Simulation, Render, and the awaited out-of-actor JPEG work.
...
| ``OfflineCaptureConfiguration``, ``OfflineCaptureAssembly``, and ``OfflineCaptureCoordinator`` | Implemented closed serial topology exposing only initial cursor plus ``POfflineCaptureTarget``; the sole effective advance authority submits each advance request at most once, validates completed identity/settings/image extent and cancellation identity, and awaits non-cancellation-inheriting JPEG work outside its actor while preserving busy backpressure and typed progress |
| ``AgentSessionConfiguration``, ``AgentSessionAssembly``, and ``AgentSessionCoordinator`` | Implemented transport-neutral live-process wrapper that privately retains the offline assembly and receives only ``POfflineCaptureTarget``; it adds mandatory expected cursors, bounded steps, reflexive-payload admission, session-qualified monotonic identity, explicit accepted high-water independent of bounded replay retention and optional next-sequence state, exact cached response replay, typed overlap/admission outcomes, and drain-before-close lifecycle |
