...
nonisolated struct RenderViewpointRevision: Codable, Comparable, Hashable, RawRepresentable, Sendable {
    static let zero = RenderViewpointRevision(rawValue: 0)

    let rawValue: UInt64

    static func < (lhs: RenderViewpointRevision, rhs: RenderViewpointRevision) -> Bool {
