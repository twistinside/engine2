...
/// Retains a Metal test submission independently of its timeout waiter.
///
...
nonisolated final class MetalTestSubmission: @unchecked Sendable {
    ...
    init(retaining retainedObjects: [AnyObject]) {
        precondition(!retainedObjects.isEmpty, "A Metal test submission must retain at least one resource owner.")
    ...
    func waitForCompletion(timeout: DispatchTime) throws {
        guard completion.wait(timeout: timeout) == .success else {
            throw MetalTestSubmissionError.timedOut
        }
        ...
        if let feedbackError {
            throw MetalTestSubmissionError.gpuExecutionFailed(
                feedbackError
            )
        }
    }
