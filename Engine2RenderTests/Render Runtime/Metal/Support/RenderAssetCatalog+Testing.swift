...
extension RenderAssetCatalog {
    /// Complete authored appearances without packaged models.
    ///
    /// Metal infrastructure tests often need the real material contract while
    /// supplying analytic geometry. Deriving every material family from
    /// `BasicGameContent` keeps one authored source of truth while avoiding
